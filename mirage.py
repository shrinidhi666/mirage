...
try:
    ...
    HAS_IMGFUNCS = False
    print
    "imgfuncs.so module not found, rotating/flipping images will be disabled."
try:
    import xmouse
    ...
    HAS_XMOUSE = False
    print
    "xmouse.so module not found, some screenshot capabilities will be disabled."
try:
    import gconf
except:
    pass
...
class Base:
    ...
    def __init__(self):
        ...
        self.action_commands = ["gimp-remote-2.4 %F", "convert %F -thumbnail 150x150 %Pt_%N.jpg",
        ...
        try:
            opts, args = getopt.getopt(sys.argv[1:], "hRvVsfo:",
                                       ["help", "version", "recursive", "verbose", "slideshow", "fullscreen",
                                        "onload="])
        ...
        if conf.has_option('actions', 'num_actions'):
            ...
            for i in range(num_actions):
                if conf.has_option('actions', 'names[' + str(i) + ']') and conf.has_option('actions', 'commands[' + str(
                        i) + ']') and conf.has_option('actions', 'shortcuts[' + str(i) + ']') and conf.has_option(
                        'actions', 'batch[' + str(i) + ']'):
        ...
        actions = (
            ...
            ('Open Remote Image', gtk.STOCK_NETWORK, _('Open _Remote image...'), None, _('Open Remote Image'),
            ...
            ('Save As', gtk.STOCK_SAVE, _('Save Image _As...'), '<Shift><Ctrl>S', _('Save Image As'),
             self.save_image_as),
            ...
            ('Previous Image', gtk.STOCK_GO_BACK, _('_Previous Image'), 'Left', _('Previous Image'),
            ...
            ('Flip Horizontally', None, _('Flip _Horizontally'), '<Ctrl>H', _('Flip Horizontally'),
            ...
            ('Exit Full Screen', leave_fullscreen_icon, _('E_xit Full Screen'), None, _('Exit Full Screen'),
             self.leave_fullscreen),
            ('Start Slideshow', gtk.STOCK_MEDIA_PLAY, _('_Start Slideshow'), 'F5', _('Start Slideshow'),
             self.toggle_slideshow),
            ('Stop Slideshow', gtk.STOCK_MEDIA_STOP, _('_Stop Slideshow'), 'F5', _('Stop Slideshow'),
             self.toggle_slideshow),
        ...
        toggle_actions = (
            ...
            ('Thumbnails Pane', None, _('Thumbnails _Pane'), None, _('Thumbnails Pane'), self.toggle_thumbpane,
             self.thumbpane_show)
        )
        ...
        self.layout.drag_dest_set(gtk.DEST_DEFAULT_HIGHLIGHT | gtk.DEST_DEFAULT_DROP, [("text/uri-list", 0, 80)],
        ...
        self.layout.add_events(
            gtk.gdk.KEY_PRESS_MASK | gtk.gdk.POINTER_MOTION_MASK | gtk.gdk.BUTTON_PRESS_MASK | gtk.gdk.BUTTON_MOTION_MASK | gtk.gdk.SCROLL_MASK)
        ...
        self.layout.add_events(
            gtk.gdk.POINTER_MOTION_MASK | gtk.gdk.POINTER_MOTION_HINT_MASK | gtk.gdk.BUTTON_RELEASE_MASK)
    ...
    def refresh_custom_actions_menu(self):
        ...
        for i in range(len(self.action_names)):
            action = [(self.action_names[i], None, self.action_names[i], self.action_shortcuts[i], None,
        ...
        uiDescription = """
			...
			"""
        for i in range(len(self.action_names)):
            uiDescription = uiDescription + """<menuitem action=\"""" + self.action_names[
                len(self.action_names) - i - 1].replace('&', '&amp;') + """\" position="top"/>"""
    ...
    def thumbpane_get_pixbuf(self, thumb_url, image_url, force_generation):
        ...
        try:
            ...
            pix.save(thumb_url, "png", {'tEXt::Thumb::URI': uri, 'tEXt::Thumb::MTime': file_mtime,
    ...
    def find_path(self, filename, exit_on_fail=True):
        ...
        if exit_on_fail:
            print
            _("Couldn't find the image %s. Please check your installation.") % filename
            sys.exit(1)
        else:
            return None
    ...
    def topwindow_keypress(self, widget, event):
        ...
        if (not (event.state & gtk.gdk.SHIFT_MASK)) and not (event.state & gtk.gdk.CONTROL_MASK) and not (
                event.state & gtk.gdk.MOD1_MASK) and not (event.state & gtk.gdk.MOD2_MASK) and not (
    ...
    def parse_action_command(self, command, batchmode):
        ...
        if batchmode:
            ...
            gobject.idle_add(self.thumbpane_set_image, self.image_list[self.curr_img_in_list], self.curr_img_in_list,
                             True)
    ...
    def recent_action_click(self, action):
        ...
        if os.path.isfile(self.recentfiles[index]) or os.path.exists(self.recentfiles[index]) or self.recentfiles[
            index].startswith('http://') or self.recentfiles[index].startswith('ftp://'):
    ...
    def parse_action_command2(self, cmd, imagename):
        ...
        if self.verbose:
            print
            _("Action return code: %s") % shell_rc
        if shell_rc != 0:
    ...
    def print_version(self):
        print
        _("Version: Mirage"), __version__
        print
        _("Website: http://mirageiv.berlios.de")
    ...
    def print_usage(self):
        ...
        ""
        print
        _("Usage: mirage [OPTION]... FILES|FOLDERS...")
        print
        ""
        print
        _("Options") + ":"
        ...
        print
        "  -v, --version        " + _("Show version information and exit")
        print
        "  -V, --verbose        " + _("Show more detailed information")
        print
        "  -R, --recursive      " + _("Recursively include all images found in")
        print
        "                       " + _("subdirectories of FOLDERS")
        print
        "  -s, --slideshow      " + _("Start in slideshow mode")
        print
        "  -f, --fullscreen     " + _("Start in fullscreen mode")
        print
        "  -o, --onload 'cmd'   " + _("Execute 'cmd' when an image is loaded")
        print
        "                       " + _("uses same syntax as custom actions,\n")
        print
        "                       " + _("i.e. mirage -o 'echo file is %F'")
    ...
    def delay_changed(self, action):
        ...
        if self.slideshow_mode:
            ...
            if self.curr_slideshow_random:
                self.timer_delay = gobject.timeout_add(int(self.curr_slideshow_delay * 1000), self.goto_random_image,
                                                       "ss")
            else:
    ...
    def expose_event(self, widget, event):
        ...
        if self.hscroll.get_property('visible'):
            try:
                ...
                newvalue = abs(
                    self.layout.get_hadjustment().get_value() * zoomratio + (self.available_image_width()) * (
                                zoomratio - 1) / 2)
                if newvalue >= self.layout.get_hadjustment().lower and newvalue <= (
                        self.layout.get_hadjustment().upper - self.layout.get_hadjustment().page_size):
        ...
        if self.vscroll.get_property('visible'):
            try:
                newvalue = abs(
                    self.layout.get_vadjustment().get_value() * zoomratio + (self.available_image_height()) * (
                                zoomratio - 1) / 2)
                if newvalue >= self.layout.get_vadjustment().lower and newvalue <= (
                        self.layout.get_vadjustment().upper - self.layout.get_vadjustment().page_size):
        ...
        if not currimg_preloaded:
            ...
            if not self.currimg_is_animation:
                ...
                if not self.currimg_pixbuf_original.get_has_alpha():
                    self.currimg_pixbuf = self.currimg_pixbuf_original.scale_simple(finalimg_width, finalimg_height,
                                                                                    self.zoom_quality)
                else:
                    ...
                    self.currimg_pixbuf = self.currimg_pixbuf_original.composite_color_simple(finalimg_width,
                                                                                              finalimg_height,
                                                                                              self.zoom_quality, 255, 8,
                                                                                              light_grey.pixel,
                                                                                              dark_grey.pixel)
            else:
    ...
    def save_image_as(self, action):
        dialog = gtk.FileChooserDialog(title=_("Save As"), action=gtk.FILE_CHOOSER_ACTION_SAVE,
    ...
    def save_image_now(self, dest_name, filetype):
        try:
            ...
            if self.filetype_is_writable(filetype):
                ...
                error_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING, gtk.BUTTONS_YES_NO,
                                                 _(
                                                     'The %s format is not supported for saving. Do you wish to save the file in a different format?') % filetype)
    ...
    def open_file_remote(self, action):
        ...
        dialog = gtk.Dialog(_("Open Remote"), self.window, gtk.DIALOG_MODAL,
    ...
    def open_file_or_folder(self, action, isfile):
        ...
        dialog = gtk.FileChooserDialog(title=_("Open"), action=gtk.FILE_CHOOSER_ACTION_OPEN,
                                       buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        if isfile:
    ...
    def update_statusbar(self):
        ...
        try:
            ...
            status_text = os.path.basename(self.currimg_name) + ":  " + str(
                self.currimg_pixbuf_original.get_width()) + "x" + str(
                self.currimg_pixbuf_original.get_height()) + "   " + str(filesize) + "KB   " + str(ratio) + "%   "
        except:
        ...
        if self.running_custom_actions:
            status_text = _('Custom actions: %(current)i of  %(total)i') % {'current': self.curr_custom_action,
    ...
    def show_custom_actions(self, action):
        ...
        self.tvcolumn1.set_max_width(
            self.actionwidget.size_request()[0] - self.tvcolumn0.get_width() - self.tvcolumn2.get_width())
        ...
        propertyinfo.set_markup('<small>' + _("Parameters") + ':\n<span font_family="Monospace">%F</span> - ' + _(
            "File path, name, and extension") + '\n<span font_family="Monospace">%P</span> - ' + _(
            "File path") + '\n<span font_family="Monospace">%N</span> - ' + _(
            "File name without file extension") + '\n<span font_family="Monospace">%E</span> - ' + _(
            "File extension (i.e. \".png\")") + '\n<span font_family="Monospace">%L</span> - ' + _(
            "List of files, space-separated") + '</small>')
        propertyinfo.set_alignment(0, 0)
        actioninfo = gtk.Label()
        actioninfo.set_markup('<small>' + _("Operations") + ':\n<span font_family="Monospace">[NEXT]</span> - ' + _(
            "Go to next image") + '\n<span font_family="Monospace">[PREV]</span> - ' + _(
            "Go to previous image") + '</small>')
    ...
    def edit_custom_action(self, button, treeview):
        ...
        if iter != None:
            ...
            self.open_custom_action_dialog(False, self.action_names[row], self.action_commands[row],
                                           self.action_shortcuts[row], self.action_batch[row], treeview)

    def open_custom_action_dialog(self, add_call, name, command, shortcut, batch, treeview):
        if add_call:
            self.dialog_name = gtk.Dialog(_("Add Custom Action"), self.actions_dialog, gtk.DIALOG_MODAL,
                                          (gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT, gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        else:
            self.dialog_name = gtk.Dialog(_("Edit Custom Action"), self.actions_dialog, gtk.DIALOG_MODAL,
                                          (gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT, gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        ...
        self.dialog_name.connect('response', self.dialog_name_response, add_call, action_name, action_command,
                                 self.shortcut, batchmode, treeview)
        self.dialog_name.run()

    def dialog_name_response(self, dialog, response, add_call, action_name, action_command, shortcut, batchmode,
                             treeview):
        if response == gtk.RESPONSE_ACCEPT:
            if not (
                    action_command.get_text() == "" or action_name.get_text() == "" or self.shortcut.get_label() == "None"):
                name = action_name.get_text()
                command = action_command.get_text()
                if ((("[NEXT]" in command.strip()) and command.strip()[-6:] != "[NEXT]") or (
                        ("[PREV]" in command.strip()) and command.strip()[-6:] != "[PREV]")):
                    error_dialog = gtk.MessageDialog(self.actions_dialog, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING,
                                                     gtk.BUTTONS_CLOSE, _(
                ...
                error_dialog = gtk.MessageDialog(self.actions_dialog, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING,
                                                 gtk.BUTTONS_CLOSE, _('Incomplete custom action specified.'))
                error_dialog.set_title(_("Invalid Custom Action"))
                error_dialog.run()
                error_dialog.destroy()
        else:
    ...
    def shortcut_clicked(self, widget):
        self.dialog_shortcut = gtk.Dialog(_("Action Shortcut"), self.dialog_name, gtk.DIALOG_MODAL,
        ...
        if shortcut[(len(shortcut) - 2):len(shortcut)] != "_L" and shortcut[(len(shortcut) - 2):len(shortcut)] != "_R":
            ...
            for i in range(len(self.keys)):
                if shortcut == self.keys[i][1]:
                    error_dialog = gtk.MessageDialog(self.dialog_shortcut, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING,
                                                     gtk.BUTTONS_CLOSE, _(
                            'The shortcut \'%(shortcut)s\' is already used for \'%(key)s\'.') % {'shortcut': shortcut,
                                                                                                 'key': self.keys[i][
                                                                                                     0]})
    ...
    def populate_treeview(self):
        ...
        for i in range(len(self.action_names)):
            if self.action_batch[i]:
                ...
                pb = None
            self.actionstore.append([pb,
                                     '<big><b>' + self.action_names[i].replace('&', '&amp;') + '</b></big>\n<small>' +
                                     self.action_commands[i].replace('&', '&amp;') + '</small>',
    ...
    def show_properties(self, action):
        ...
        imagesize2 = gtk.Label(
            str(self.currimg_pixbuf_original.get_width()) + "x" + str(self.currimg_pixbuf_original.get_height()))
    ...
    def show_prefs(self, action):
        ...
        openallimages.set_tooltip_text(_(
        ...
        hiddenimages.set_tooltip_text(
            _("If checked, Mirage will open hidden files. Otherwise, hidden files will be ignored."))
        ...
        randomize.set_tooltip_text(
            _("If enabled, a random image will be chosen during slideshow mode (without loading any image twice)."))
        ...
        disable_screensaver.set_tooltip_text(
            _("If enabled, xscreensaver will be temporarily disabled during slideshow mode."))
        ...
        if len(self.image_list) > 0:
            ...
            if response == gtk.RESPONSE_ACCEPT:
                try:
                    ...
                    error_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING, gtk.BUTTONS_OK,
    ...
    def delete_image(self, action):
        ...
        if len(self.image_list) > 0:
            ...
            if self.confirm_delete:
                permlabel = gtk.Label(
                    _('Are you sure you wish to permanently delete %s?') % os.path.split(self.currimg_name)[1])
            ...
            if response == gtk.RESPONSE_YES:
                try:
                    ...
                    if len(self.image_list) >= 1:
                        ...
                        self.preloadimg_prev_in_list = -1
                        self.preloadimg_next_in_list = -1
                        self.load_when_idle = gobject.idle_add(self.load_new_image, False, False, True, True, True,
                                                               True)
                        self.set_go_navigation_sensitivities(False)
                    else:
    ...
    def defaultdir_clicked(self, button):
        getdir = gtk.FileChooserDialog(title=_("Choose directory"), action=gtk.FILE_CHOOSER_ACTION_OPEN,
                                       buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OPEN, gtk.RESPONSE_OK))
    ...
    def show_about(self, action):
        ...
        self.about_dialog.set_authors(
            ['Scott Horowitz <stonecrest@gmail.com>', 'Fredric Johansson <fredric.miscmail@gmail.com>'])
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                error_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING,
                                                                 gtk.BUTTONS_CLOSE,
    ...
    def button_pressed(self, widget, event):
        if self.image_loaded:
            ...
            if (event.button == 2 or event.button == 1) and (
                    self.hscroll.get_property('visible') == True or self.vscroll.get_property('visible') == True):
    ...
    def zoom_to_fit_window(self, action, is_preloadimg_next, is_preloadimg_prev):
        if is_preloadimg_next:
            ...
            if self.currimg_name != "" and (
                    self.slideshow_mode or self.UIManager.get_widget('/MainMenu/ViewMenu/Fit').get_property(
                    'sensitive')):
                self.image_zoomed = True
    ...
    def zoom_1_to_1(self, action, is_preloadimg_next, is_preloadimg_prev):
        if is_preloadimg_next:
            ...
            if self.currimg_name != "" and (self.slideshow_mode or self.currimg_is_animation or (
                    not self.currimg_is_animation and self.UIManager.get_widget('/MainMenu/ViewMenu/1:1').get_property(
                    'sensitive'))):
                self.image_zoomed = True
    ...
    def get_pixbuf_of_size(self, pixbuf, size, zoom_quality):
        ...
        if not pixbuf.get_has_alpha():
            ...
            light_grey = colormap.alloc_color('#666666', True, True)
            dark_grey = colormap.alloc_color('#999999', True, True)
            crop_pixbuf = pixbuf.composite_color_simple(image_width, image_height, zoom_quality, 255, 8,
                                                        light_grey.pixel, dark_grey.pixel)
    ...
    def crop_image(self, action):
        ...
        vbox_right = gtk.VBox()
        width_adj = gtk.Adjustment(self.currimg_pixbuf_original.get_width(), 1,
                                   self.currimg_pixbuf_original.get_width(), 1, 10, 0)
        ...
        height_adj = gtk.Adjustment(self.currimg_pixbuf_original.get_height(), 1,
                                    self.currimg_pixbuf_original.get_height(), 1, 10, 0)
        ...
        dialog.vbox.show_all()
        image.set_events(
            gtk.gdk.POINTER_MOTION_MASK | gtk.gdk.POINTER_MOTION_HINT_MASK | gtk.gdk.BUTTON_PRESS_MASK | gtk.gdk.BUTTON_MOTION_MASK | gtk.gdk.BUTTON_RELEASE_MASK)
        image.connect("expose-event", self.crop_image_expose_cb, crop_pixbuf, image_width, image_height)
        image.connect("motion_notify_event", self.crop_image_mouse_moved, image, 0, 0, x, y, width, height, image_width,
                      image_height, width_adj, height_adj)
        ...
        self.x_changed = x.connect('value-changed', self.crop_value_changed, x, y, width, height, width_adj, height_adj,
                                   image_width, image_height, image, 0)
        self.y_changed = y.connect('value-changed', self.crop_value_changed, x, y, width, height, width_adj, height_adj,
                                   image_width, image_height, image, 1)
        self.width_changed = width.connect('value-changed', self.crop_value_changed, x, y, width, height, width_adj,
                                           height_adj, image_width, image_height, image, 2)
        self.height_changed = height.connect('value-changed', self.crop_value_changed, x, y, width, height, width_adj,
                                             height_adj, image_width, image_height, image, 3)
        image.realize()
        ...
        if response == gtk.RESPONSE_ACCEPT:
            dialog.destroy()
            if self.rect != None:
                temp_pixbuf = gtk.gdk.Pixbuf(gtk.gdk.COLORSPACE_RGB, self.currimg_pixbuf_original.get_has_alpha(), 8,
                                             self.coords[2], self.coords[3])
                self.currimg_pixbuf_original.copy_area(self.coords[0], self.coords[1], self.coords[2], self.coords[3],
                                                       temp_pixbuf, 0, 0)
    ...
    def crop_value_changed(self, currspinbox, x, y, width, height, width_adj, height_adj, image_width, image_height,
                           image, type):
        if type == 0:  # X
        ...
        self.crop_rectangle[0] = int(
            round(float(self.coords[0]) / self.currimg_pixbuf_original.get_width() * image_width, 0))
        self.crop_rectangle[1] = int(
            round(float(self.coords[1]) / self.currimg_pixbuf_original.get_height() * image_height, 0))
        x2 = int(round(float(self.coords[2]) / self.currimg_pixbuf_original.get_width() * image_width, 0)) + \
             self.crop_rectangle[0]
        y2 = int(round(float(self.coords[3]) / self.currimg_pixbuf_original.get_height() * image_height, 0)) + \
             self.crop_rectangle[1]
        self.drawing_crop_rectangle = True
        self.update_rectangle = True
        self.crop_image_mouse_moved(None, None, image, x2, y2, x, y, width, height, image_width, image_height,
    ...
    def crop_image_mouse_moved(self, widget, event, image, x2, y2, x, y, width, height, image_width, image_height,
                               width_adj, height_adj):
        if event != None:
        ...
        if self.drawing_crop_rectangle:
            if self.crop_rectangle != None or self.update_rectangle:
                ...
                if event != None:
                    self.coords = [0, 0, 0, 0]
                    self.coords[0] = int(
                        round(float(self.rect[0]) / image_width * self.currimg_pixbuf_original.get_width(), 0))
                    self.coords[1] = int(
                        round(float(self.rect[1]) / image_height * self.currimg_pixbuf_original.get_height(), 0))
                    self.coords[2] = int(
                        round(float(self.rect[2]) / image_width * self.currimg_pixbuf_original.get_width(), 0))
                    self.coords[3] = int(
                        round(float(self.rect[3]) / image_height * self.currimg_pixbuf_original.get_height(), 0))
    ...
    def resize_image(self, action):
        ...
        if response == gtk.RESPONSE_ACCEPT:
            ...
            dialog.destroy()
            self.currimg_pixbuf_original = self.currimg_pixbuf_original.scale_simple(pixelwidth, pixelheight,
                                                                                     self.zoom_quality)
            self.load_new_image2(False, True, False, False)
            self.image_modified = True
        else:
    ...
    def goto_image(self, location, action):
        ...
        if self.slideshow_mode and action != "ss":
            gobject.source_remove(self.timer_delay)
        if ((location == "PREV" or location == "NEXT" or location == "RANDOM") and len(self.image_list) > 1) or (
                location == "FIRST" and (len(self.image_list) > 1 and self.curr_img_in_list != 0)) or (
                location == "LAST" and (
                len(self.image_list) > 1 and self.curr_img_in_list != len(self.image_list) - 1)) or valid_int(location):
            self.load_new_image_stop_now()
            cancel = self.autosave_image()
            if cancel:
                return
            ...
            if check_wrap:
                if self.listwrap_mode == 0:
                    ...
                    if location == "PREV":
                        dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                                                   gtk.MESSAGE_QUESTION, gtk.BUTTONS_YES_NO, _(
            ...
            if self.slideshow_mode:
                if self.curr_slideshow_random:
                    self.timer_delay = gobject.timeout_add(int(self.curr_slideshow_delay * 1000),
                                                           self.goto_random_image, "ss")
                else:
                    self.timer_delay = gobject.timeout_add(int(self.curr_slideshow_delay * 1000), self.goto_next_image,
                                                           "ss")
            gobject.idle_add(self.thumbpane_select, self.curr_img_in_list)
    ...
    def load_new_image(self, check_prev_last, use_current_pixbuf_original, reset_cursor, perform_onload_action,
    ...
    def load_new_image2(self, check_prev_last, use_current_pixbuf_original, reset_cursor, perform_onload_action,
        ...
        if used_prev or used_next:
            ...
            if self.open_mode == self.open_mode_smart or (
                    self.open_mode == self.open_mode_last and self.last_mode == self.open_mode_smart):
                self.last_image_action_was_fit = True
                self.last_image_action_was_smart_fit = True
            elif self.open_mode == self.open_mode_fit or (
                    self.open_mode == self.open_mode_last and self.last_mode == self.open_mode_fit):
                self.last_image_action_was_fit = True
                self.last_image_action_was_smart_fit = False
            elif self.open_mode == self.open_mode_1to1 or (
                    self.open_mode == self.open_mode_last and self.last_mode == self.open_mode_1to1):
                self.last_image_action_was_fit = False
        else:
        ...
        try:
            if self.preloading_images and len(self.image_list) > 1:
                ...
                self.preloadimg_next_width = int(
                    self.preloadimg_next_pixbuf_original.get_width() * self.preloadimg_next_zoomratio)
                self.preloadimg_next_height = int(
                    self.preloadimg_next_pixbuf_original.get_height() * self.preloadimg_next_zoomratio)
                if not self.preloadimg_next_is_animation:
                    ...
                    if not self.preloadimg_next_pixbuf_original.get_has_alpha():
                        self.preloadimg_next_pixbuf = self.preloadimg_next_pixbuf_original.scale_simple(
                            self.preloadimg_next_width, self.preloadimg_next_height, self.zoom_quality)
                    else:
                        colormap = self.imageview.get_colormap()
                        light_grey = colormap.alloc_color('#666666', True, True)
                        dark_grey = colormap.alloc_color('#999999', True, True)
                        self.preloadimg_next_pixbuf = self.preloadimg_next_pixbuf_original.composite_color_simple(
                            self.preloadimg_next_width, self.preloadimg_next_height, self.zoom_quality, 255, 8,
                            light_grey.pixel, dark_grey.pixel)
                else:
                ...
                if self.verbose:
                    print
                    _("Preloading: %s") % self.preloadimg_next_name
        except:
            self.preloadimg_next_in_list = -1
        ...
        try:
            if self.preloading_images and len(self.image_list) > 1:
                ...
                self.preloadimg_prev_width = int(
                    self.preloadimg_prev_pixbuf_original.get_width() * self.preloadimg_prev_zoomratio)
                self.preloadimg_prev_height = int(
                    self.preloadimg_prev_pixbuf_original.get_height() * self.preloadimg_prev_zoomratio)
                if not self.preloadimg_prev_is_animation:
                    ...
                    if not self.preloadimg_prev_pixbuf_original.get_has_alpha():
                        self.preloadimg_prev_pixbuf = self.preloadimg_prev_pixbuf_original.scale_simple(
                            self.preloadimg_prev_width, self.preloadimg_prev_height, self.zoom_quality)
                    else:
                        colormap = self.imageview.get_colormap()
                        light_grey = colormap.alloc_color('#666666', True, True)
                        dark_grey = colormap.alloc_color('#999999', True, True)
                        self.preloadimg_prev_pixbuf = self.preloadimg_prev_pixbuf_original.composite_color_simple(
                            self.preloadimg_prev_width, self.preloadimg_prev_height, self.zoom_quality, 255, 8,
                            light_grey.pixel, dark_grey.pixel)
                else:
                ...
                if self.verbose:
                    print
                    _("Preloading: %s") % self.preloadimg_prev_name
        except:
            self.preloadimg_prev_in_list = -1
    ...
    def expand_filelist_and_load_image(self, inputlist):
        ...
        for item in inputlist:
            if not self.closing_app:
                if os.path.isfile(item):
                    if self.valid_image(item):
                        ...
                        if self.verbose:
                            self.images_found += 1
                            print
                            _("Found: %(item)s [%(number)i]") % {'item': item, 'number': self.images_found}
                else:
                ...
                if second_image_found and not second_image_preloaded and ((
                                                                                  not second_image_came_from_dir and self.curr_img_in_list + 1 <= len(
                                                                                  self.image_list) - 1) or second_image_came_from_dir):
    ...
    def expand_directory(self, item, stop_when_second_image_found, go_buttons_enabled, update_window_title,
                         print_found_msg):
        if not self.stop_now and not self.closing_app:
            ...
            for item2 in os.listdir(item):
                if not self.closing_app and not self.stop_now:
                    ...
                    if (not self.open_hidden_files and os.path.basename(item_fullpath2)[
                        0] != '.') or self.open_hidden_files:
            ...
            if len(folderlist) > 0:
                ...
                for item2 in folderlist:
                    if not self.stop_now:
                        self.expand_directory(item2, stop_when_second_image_found, go_buttons_enabled,
                                              update_window_title, print_found_msg)
    ...
    def image_flip(self, old_pix, vertical):
        ...
        d = None
        if vertical:
            d, w, h, rws = imgfuncs.vert(old_pix.get_pixels(), width, height, old_pix.get_rowstride(),
                                         old_pix.get_n_channels())
        else:
            d, w, h, rws = imgfuncs.horiz(old_pix.get_pixels(), width, height, old_pix.get_rowstride(),
                                          old_pix.get_n_channels())
        if d:
            new_pix = gtk.gdk.pixbuf_new_from_data(d, old_pix.get_colorspace(), old_pix.get_has_alpha(),
                                                   old_pix.get_bits_per_sample(), w, h, rws)
            return new_pix
        return old_pix
    ...
    def image_rotate(self, old_pix, full_angle):
        ...
        if angle:
            d = None
            if angle % 270 == 0:
                d, w, h, rws = imgfuncs.right(old_pix.get_pixels(), width, height, old_pix.get_rowstride(),
                                              old_pix.get_n_channels())
            elif angle % 180 == 0:
                d, w, h, rws = imgfuncs.mirror(old_pix.get_pixels(), width, height, old_pix.get_rowstride(),
                                               old_pix.get_n_channels())
            elif angle % 90 == 0:
                d, w, h, rws = imgfuncs.left(old_pix.get_pixels(), width, height, old_pix.get_rowstride(),
                                             old_pix.get_n_channels())
            if d:
                new_pix = gtk.gdk.pixbuf_new_from_data(d, old_pix.get_colorspace(), old_pix.get_has_alpha(),
                                                       old_pix.get_bits_per_sample(), w, h, rws)
    ...
    def update_title(self):
        if len(self.image_list) == 0:
            ...
            title = "Mirage - " + _("[%(current)i of %(total)i]") % {'current': self.curr_img_in_list + 1, 'total': len(
                self.image_list)} + ' ' + os.path.basename(self.currimg_name)
            if self.slideshow_mode:
